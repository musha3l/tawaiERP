...
class JournalEntry(AccountsController):
	...
	def vat_fields_mandatory(self):
		for d in self.get("accounts"):
			account_type = frappe.db.get_value("Account", d.account, "account_type")
			if account_type == "Tax":
				if not d.supplier or not d.supplier_invoice_date or not d.supplier_invoice_no or not d.supplier_vat_id or not d.vat_value2:
					frappe.throw(_("Row {0}: Supplier & Supplier Invoice Date & Supplier Invoice No & Supplier VAT ID & VAT% is required for account {1}").format(d.idx, d.account))
...
def reset_t_pe():
	...
	#get_all Doctype
	je_list = frappe.get_list("Payment Entry",order_by='name asc')
	for je in je_list:
		print (je.name)

	# delete all Enahanced Naming recored for doc
	namming =frappe.get_list("Enhanced Nameing Doc", fields=["name","name_of_doc", "index_value","year"],filters={"name_of_doc":"Payment Entry"},ignore_permissions=True)
	for n in namming:
		nammeing_doc = frappe.get_doc("Enhanced Nameing Doc",n["name"])
		print (nammeing_doc.name)
		nammeing_doc.delete()
	...
	for je in je_list :
		print(je.name)
		je_doc = frappe.get_doc("Payment Entry",je["name"])
		last_title = "Not Set"
		namming =frappe.get_list("Enhanced Nameing Doc", fields=["name","name_of_doc", "index_value","year"],
			filters={"year": str(getdate(je_doc.posting_date).year),
			"name_of_doc":je_doc.doctype},ignore_permissions=True)
