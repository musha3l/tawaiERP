...
def get_columns(filters):
	...
	columns += [
		...
		_("Title") + "::120",
		_("Against Account") + "::120", _("Party Type") + "::80", _("Party") + "::150",_("Party Name") + "::150",
...
def get_result_as_list(data, filters):
	supplier_details =  get_supplier_details(filters)
	customer_details =  get_customer_details(filters)
	employee_details =  get_employee_details(filters)

	result = []
	for d in data:
		if(d.get("party_type") == "Supplier"):
			party = supplier_details.get(d.get("party")) or frappe._dict()
		elif(d.get("party_type") == "Customer"):
			party = customer_details.get(d.get("party")) or frappe._dict()
		else:
			party = employee_details.get(d.get("party")) or frappe._dict()
		...
		row += [(d.get("voucher_type")), (d.get("voucher_no")), _(d.get("title")), _(d.get("against")),
			_(d.get("party_type")), _(d.get("party")), party.party_name,_(d.get("project")), _(d.get("cost_center")), _(d.get("against_voucher_type")), _(d.get("against_voucher")), _(d.get("remarks")), _(d.get("reason")), _(d.get("description")),_(d.get("purchase_order"))
		]
...
def get_supplier_details(filters):
	supplier_details = {}
	dt = "Supplier"
	for t in frappe.db.sql("select name, supplier_name as party_name from `tab{0}`".format(dt), as_dict=1):
		supplier_details[t.name] = t
...
def get_customer_details(filters):
	customer_details = {}
	dt = "Customer"
	for t in frappe.db.sql("select name, customer_name as party_name from `tab{0}`".format(dt), as_dict=1):
		customer_details[t.name] = t
...
def get_employee_details(filters):
	employee_details = {}
	dt = "Employee"
	for t in frappe.db.sql("select name, employee_name as party_name from `tab{0}`".format(dt), as_dict=1):
		employee_details[t.name] = t
