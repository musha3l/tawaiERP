...
@frappe.whitelist()
def email_salary_slips(employee_list,start_date,end_date):
	employee_list = json.loads(employee_list)
	for employee in employee_list:
		# frappe.msgprint(employee['employee'])
		salary_slip = frappe.db.sql("select name from `tabSalary Slip` where employee='{0}' and start_date='{1}' and end_date='{2}' and docstatus=1".format(employee['employee'],start_date,end_date))
		if salary_slip:
			emp = frappe.db.sql("select name from `tabEmployee` where employee='{0}'".format(employee['employee']))
			doc_emp = frappe.get_doc("Employee", emp[0][0])
			# frappe.msgprint(doc_emp.prefered_email)
			receiver = doc_emp.prefered_email
			# receiver = 'msha6377@gmail.com'
			doc = frappe.get_doc("Salary Slip", salary_slip[0][0])
			if receiver:
			    subj = 'Salary Slip - from {0} to {1}'.format(start_date,end_date)
			    frappe.sendmail([receiver], subject=subj, message = _("Please see attachment"),
			    attachments=[frappe.attach_print('Salary Slip', salary_slip[0][0], file_name=salary_slip[0][0])], reference_doctype= 'Salary Slip', reference_name= salary_slip[0][0])
			    doc.email_salary_slip = 1
			    doc.save()
			    frappe.msgprint("{0} - Mail Send".format(employee['employee_name']))
			else:
			    frappe.msgprint(_("{0} - Employee email not found, hence email not sent").format(employee['employee_name']))
		else:
			frappe.msgprint("{0} - No salary slip found to email it for the above selected criteria".format(employee['employee_name']))


@frappe.whitelist()
def get_employees(start_date,end_date, department=None, branch=None, company=None):
	email_not_marked = []
	email_marked = []
	...
	marked_employee = {}
	for emp in frappe.get_list("Salary Slip", fields=["employee", "email_salary_slip"],
							   filters={"start_date": start_date,"end_date": end_date,"email_salary_slip":1}):
		marked_employee[emp['employee']] = emp['email_salary_slip']

	for employee in employee_list:
		employee['email_salary_slip'] = marked_employee.get(employee['employee'])
		if 'EMP/1' in employee['employee'] or employee['employee'] =='EMP/2007':
			if employee['employee'] not in marked_employee:
				email_not_marked.append(employee)
			else:
				email_marked.append(employee)
	return {
		"marked": email_marked,
		"unmarked": email_not_marked
	}
