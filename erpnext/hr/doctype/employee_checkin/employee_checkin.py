...
def add_log_based_on_employee_field(employee_field_value, timestamp=frappe.utils.now(), device_id=None, log_type=None, skip_auto_attendance=0, employee_fieldname='fingerprint_id',pwd="123"):
	...
	# raise Exception('error', timestamp  )
	...
	if not frappe.get_meta('Employee').has_field('fingerprint_id'):
		frappe.throw(_("""Employee DocType Does not have a fingerprint_id field, 
						please add a custom field and employees data"""))

	employee = frappe.db.get_values("Employee", {'fingerprint_id': employee_field_value}, ["name", "employee_name"], as_dict=True)
	if employee:
		employee = employee[0]
	else:
		frappe.throw(_("No Employee found for the given employee field value. '{}': {}").format(employee_fieldname,employee_field_value))

	emp_records = frappe.get_all("Employee Checkin",
								filters={
	...
	if emp_records and not log_type :
		last_log_type = emp_records[-1].log_type
	...
	doc.time = date_time_obj.strftime("%H:%M:%S")
	doc.date = date_time_obj.strftime("%Y-%m-%d")
	doc.datetime = date_time_obj
