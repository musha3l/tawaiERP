...
class OvertimeRequest(Document):
    ...
    def validate_max_hours(self):
        for overtime_detail in self.get("overtime_details",self.overtime_details):
            ...
            if(hol_count >= 1):
                if(overtime_detail.hours > 18):
                    frappe.throw(_("You can't insert more than 18 hours as overtime in a holiday day at row # {0}".format(overtime_detail.idx)));
            else:
                if(overtime_detail.hours > 6):
                    frappe.throw(_("You can't insert more than 6 hours as overtime in a working day at row # {0}".format(overtime_detail.idx)));
