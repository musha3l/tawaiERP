...
class SalarySlip(TransactionBase):
    ...
    def validate_overtime(self):
        ...
        if prev_month != 12:
            prev_month_start_date = "{0}-{1}-21".format(getdate(self.start_date).year, prev_month)
        else:
            prev_month_start_date = "{0}-{1}-21".format(getdate(self.start_date).year - 1, prev_month)

        current_month_end_date = "{0}-{1}-20".format(getdate(self.start_date).year,getdate(self.start_date).month)
        overtime_hours = frappe.db.sql("""Select total_modified_hours from `tabOvertime Request` where
         modified between '{0}' and '{1}' and employee = '{2}' and docstatus = 1"""
        .format(prev_month_start_date,current_month_end_date, self.employee))
        if overtime_hours:
            total_hours =0
            for houres in overtime_hours:
                total_hours += houres[0]
    ...
    def validate_business_trip_earning(self):
        ...
        if emp_return_date:
            salary_end_date = datetime.datetime.strptime(str(self.start_date), '%Y-%m-%d')
            return_date = datetime.datetime.strptime(str(emp_return_date[0][0]), '%Y-%m-%d')

            self.append(
                'earnings', {
                "salary_component": 'Business Trip Per Diem',
                "amount": emp_return_date[0][1] - emp_return_date[0][2]
            })

            self.append(
                'earnings', {
                "salary_component": 'Business Trip Ticket',
                "amount": emp_return_date[0][2]
            })


    def validate_training_trip_earning(self):
        emp_return_date= frappe.db.sql("""
            select from_date,cost_total,ticket_cost from `tabTraining Trip` where
        ...
        if emp_return_date:
            ...
            self.append(
                'earnings', {
                "salary_component": 'Training Trip Per Diem',
                "amount": emp_return_date[0][1] - emp_return_date[0][2]
            })

            self.append(
                'earnings', {
                "salary_component": 'Training Trip Ticket',
                "amount": emp_return_date[0][2]
            })
...
def get_permission_query_conditions(user):
    ...
    if employees:
        ...
        if u'HR Specialist' in frappe.get_roles(user) or u'HR Manager' in frappe.get_roles(user) or u'Accounts User' in frappe.get_roles(user):
            query=""
        return query
